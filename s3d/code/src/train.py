...
# Validation function.
def validate_by_classes(model, testloader, criterion):
    model.eval()
    print('Validation')
    valid_running_loss = 0.0
    valid_running_correct = 0
    ...
    with torch.no_grad():
        for i, data in enumerate(prog_bar):
            ...
            if labels not in acc_by_classes:
                acc_by_classes[labels] = [0, 0]

            if (preds == labels).sum().item() :
                acc_by_classes[labels][0] += 1
            acc_by_classes[labels][1] += 1
    ...
    return epoch_loss, epoch_acc, acc_by_classes
...
if __name__ == '__main__':
    ...
    dataset_train = VideoClassificationDataset(
        os.path.join('..', '..', 'full_workout_dataset', 'split'),
